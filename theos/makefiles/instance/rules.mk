...
ifneq ($(TARGET_CODESIGN),)
	...
	@cp "$$@" "./$(1).$(THEOS_CURRENT_ARCH).jtool.unsigned"
	jtool --sign --inplace $($(THEOS_CURRENT_INSTANCE)_CODESIGN_FLAGS) "$(1).$(THEOS_CURRENT_ARCH).jtool.unsigned"
	jtool --sign --inplace $($(THEOS_CURRENT_INSTANCE)_CODESIGN_FLAGS) "$(1).$(THEOS_CURRENT_ARCH).jtool.unsigned"
	@mv "$(1).$(THEOS_CURRENT_ARCH).jtool.unsigned" "$$@"
