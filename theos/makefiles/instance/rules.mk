...
.INTERMEDIATE: $$(THEOS_OBJ_DIR)/$(1).$(_THEOS_OUT_FILE_TAG).unsigned
$(THEOS_OBJ_DIR)/$(1): $$(THEOS_OBJ_DIR)/$(1).$(_THEOS_OUT_FILE_TAG).unsigned
	@mkdir -p $(_THEOS_TMP_FOR_WSL)/$(THEOS_CURRENT_INSTANCE)
	$$(ECHO_SIGNING)cp "$$<" $(_THEOS_TMP_FOR_WSL)/$(THEOS_CURRENT_INSTANCE)/ && $$(_THEOS_CODESIGN_COMMANDLINE) "$(_THEOS_TMP_FOR_WSL)/$(THEOS_CURRENT_INSTANCE)/$(1).$(_THEOS_OUT_FILE_TAG).unsigned" && mv "$(_THEOS_TMP_FOR_WSL)/$(THEOS_CURRENT_INSTANCE)/$(1).$(_THEOS_OUT_FILE_TAG).unsigned" "$$@"$$(ECHO_END)
	@# $$(ECHO_SIGNING)$$(_THEOS_CODESIGN_COMMANDLINE) "$$<" && mv "$$<" "$$@"$$(ECHO_END)
...
ifeq ($$(OBJ_FILES_TO_LINK),)
	...
	$$(ECHO_LINKING)$$(ECHO_UNBUFFERED)$$(TARGET_LD) $$(ALL_LDFLAGS) -o "$$@" $$^ | (grep -v 'usr/lib/dylib1.o, missing required architecture' || true)$$(ECHO_END)
ifeq ($(SHOULD_STRIP),$(_THEOS_TRUE))
	@mkdir -p "$(_THEOS_TMP_FOR_WSL)/$(THEOS_CURRENT_INSTANCE)/$(THEOS_CURRENT_ARCH)"
	$$(ECHO_STRIPPING)$$(ECHO_UNBUFFERED)cp "$$@" "$(_THEOS_TMP_FOR_WSL)/$(THEOS_CURRENT_INSTANCE)/$(THEOS_CURRENT_ARCH)" && $$(TARGET_STRIP) $$(ALL_STRIP_FLAGS) "$(_THEOS_TMP_FOR_WSL)/$(THEOS_CURRENT_INSTANCE)/$(THEOS_CURRENT_ARCH)/$(1)" && mv "$(_THEOS_TMP_FOR_WSL)/$(THEOS_CURRENT_INSTANCE)/$(THEOS_CURRENT_ARCH)/$(1)" $$(THEOS_OBJ_DIR)$$(ECHO_END)
	@# $$(ECHO_STRIPPING)$$(ECHO_UNBUFFERED)$$(TARGET_STRIP) $$(ALL_STRIP_FLAGS) "$$@"$$(ECHO_END)
endif
endif
endif
